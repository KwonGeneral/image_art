...
import com.kwon.image_art.PieChart
import com.kwon.image_art.Slice
import com.kwon.image_art.LegendAdapter
import com.kwon.image_art.p2d
import kotlin.math.abs
import kotlin.math.atan2
import kotlin.math.cos
import kotlin.math.sin

class ClickPieChart @JvmOverloads constructor(
    context: Context,
    ...
    private fun initAttributes(attrs: AttributeSet?) {
        ...
        try {
            popupText = typedArray.getString(R.styleable.PieChart_popupText) ?: ""

            showPercentage =
                typedArray.getBoolean(R.styleable.PieChart_showPercentage, false)

            animationDuration =
                abs(typedArray.getInt(R.styleable.PieChart_animationDuration, 0))

            centerPaint.color = typedArray.getColor(
                R.styleable.PieChart_centerColor,
                ContextCompat.getColor(context, android.R.color.white)
            )

            showPopup = typedArray.getBoolean(R.styleable.PieChart_showPopup, true)
        } finally {
            typedArray.recycle()
        }
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        val centerX = (measuredWidth / 4).toFloat()
        val centerY = (measuredHeight / 4).toFloat()
            ...
            slices?.forEach { slice ->
                slice.arc?.let { arc ->
                    if (currentSweepAngle > arc.startAngle + arc.sweepAngle) {
                        slicePaint.color = Color.parseColor(java.lang.String.format("#%06X", 0xFFFFFF and slice.color))
                        canvas?.drawArc(
                            rectF!!,
    ...
    private fun showInfoPopup(index: Int) {
        ...
        ImageViewCompat.setImageTintList(
            ...
            ColorStateList.valueOf(slices?.get(index)?.color ?: R.color.semiGray)
        )
